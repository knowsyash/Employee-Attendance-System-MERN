...
try {
    ...
    $dept = if ($loginResponse.user.department) { $loginResponse.user.department } else { "N/A" }
    Write-Host "  Department: $dept"
    $pos = if ($loginResponse.user.position) { $loginResponse.user.position } else { "N/A" }
    Write-Host "  Position: $pos"
    $empId = if ($loginResponse.user.employeeId) { $loginResponse.user.employeeId } else { "N/A" }
    Write-Host "  Employee ID: $empId"
    $status = if ($loginResponse.user.isActive) { "Active" } else { "Inactive" }
    Write-Host "  Status: $status"
    $tokenPreview = $loginResponse.token.Substring(0, [Math]::Min(20, $loginResponse.token.Length))
    ...
    try {
        ...
        $checkInResponse = Invoke-RestMethod -Uri "$BaseURL/api/attendance/checkin" -Method POST -Headers $headers -Body "{}" -ErrorAction Stop

        Write-Host "[SUCCESS] Check-in successful!" -ForegroundColor Green
        ...
        if ($_.Exception.Response.StatusCode.value__ -eq 400) {
            Write-Host "[WARNING] Already checked in today" -ForegroundColor Yellow
            Write-Host ""
        }
        else {
            Write-Host "[ERROR] Check-in failed: $($_.Exception.Message)" -ForegroundColor Red
            Write-Host ""
        }
    }

    # Step 3: Test Get Today's Attendance
    Write-Host "[3] TESTING GET TODAYS ATTENDANCE..." -ForegroundColor Blue -BackgroundColor Black
    Write-Host ""

    try {
        $attendanceResponse = Invoke-RestMethod -Uri "$BaseURL/api/attendance/today" -Method GET -Headers $headers -ErrorAction Stop

        Write-Host "[SUCCESS] Retrieved todays attendance!" -ForegroundColor Green
        ...
        if ($_.Exception.Response.StatusCode.value__ -eq 404) {
            Write-Host "[WARNING] No attendance record for today" -ForegroundColor Yellow
            Write-Host ""
        }
        else {
            Write-Host "[ERROR] Failed to get attendance: $($_.Exception.Message)" -ForegroundColor Red
            Write-Host ""
        }
    }
    ...
    # Summary
    Write-Host "=======================================================" -ForegroundColor Cyan
    Write-Host "[SUCCESS] PROTOCOL TEST COMPLETED!" -ForegroundColor Green
    Write-Host "=======================================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Summary:" -ForegroundColor Green
    Write-Host "  [OK] Login authentication working"
    Write-Host "  [OK] Token generation successful"
    Write-Host "  [OK] Super admin account verified"
    Write-Host ""
